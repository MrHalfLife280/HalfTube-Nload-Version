...
if (!$user_id) {
    // Redirect if not logged in (before any output or HTML)
...
if (isset($_GET['id'])) {
    ...
    // Increment view count once per session for the user
    if (!isset($_SESSION['viewed_videos'][$videoId])) {
        $stmt = $pdo->prepare("UPDATE videos SET views = views + 1 WHERE id = ?");
        $stmt->execute([$videoId]);
        $_SESSION['viewed_videos'][$videoId] = true; // Mark the video as viewed in the session
    }

    // Fetch uploader details (username and video count)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment'], $_POST['video_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM comments WHERE video_id = ? AND user_id = ?");
    $stmt->execute([$video_id, $user_id]);
    $existingComment = $stmt->fetchColumn();

    if ($existingComment > 0) {
        // User already commented
        echo "You have already commented on this video.";
        exit();
    }

    // Insert the comment into the database with the user_id (from session)
    if (!empty($commentText)) {
        $stmt = $pdo->prepare("INSERT INTO comments (video_id, user_id, comment, date_posted) VALUES (?, ?, ?, NOW())");
        $stmt->execute([$video_id, $user_id, $commentText]);

        // Redirect back to the video page after posting the comment
        header("Location: watch.php?id=" . urlencode($video_id));
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rating'])) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM ratings WHERE video_id = ? AND user_id = ?");
    $stmt->execute([$videoId, $user_id]);
    ...
    $stmt = $pdo->prepare("INSERT INTO ratings (video_id, user_id, rating) VALUES (?, ?, ?)");
    $stmt->execute([$videoId, $user_id, $rating]);
	...
	<tr valign="top">
		...
		<table width="425" cellpadding="0" cellspacing="0" border="0" align="center">
            ...
            ?>					</div>
					...
					Added: <?php echo date("F j, Y", strtotime($video['upload_date'])); ?> by <a href="profile.php?user=<?php echo htmlspecialchars($user['username']); ?>" <?php echo htmlspecialchars($user['username']); ?> </a> //
		...
		<table width="280" align="center" cellpadding="0" cellspacing="0" border="0" bgcolor="#CCCCCC">
			...
			<tr>
				...
				<table width="270" cellpadding="0" cellspacing="0" border="0">
					<tr valign="top">
						<td><div class="moduleFrameBarTitle">Tag // aerobatic sukhoi airplane stunt trick (71)</div></td>
